...
def get_connections(network, user):
    return get_property(network, user, conn_str)
...
def get_games_liked(network, user):
    return get_property(network, user, game_str)
...
def add_new_user(network, user, games):
    if user not in network:
        network[user] = {}
        network[user][conn_str] = []
        network[user][game_str] = games
    return network
...
def get_secondary_connections(network, user):
    ...
    for friend in get_connections(network, user):
        for foaf in get_connections(network, friend):
            if foaf not in output: 
                output.append(foaf)
    return output
